...
#Reads in all the data from project datasets
...
#Handles issues with improper strings and adjusts kicking columns from format of attempts
#Made to their own columns of attempts and made
#Adds decision column of whether team went for it or not and dropped unnecessary columns
def adjust_data_issues(df2018):
    ...
    # creates new separate columns for kicks attempted vs kicks made
    for i in range(len(new_kicking_columns)):
        df2018[new_kicking_columns[i]] = df2018[old_kicking_columns[i]].str.split('_', expand=True).dropna().astype('int')
...
def adjust_main_df():
    ...
    print("Filtering to only 2018 and fourth downs...")
    df2018 = filter_df(df)
    print("Combine main DF, offense, defense DF...")
    df2018 = merge_all_df(df2018,off_down,spec_teams,off_rush, off_pass,def_down,def_pass,def_rush)
    print("Merge with parts of 2017 data")
